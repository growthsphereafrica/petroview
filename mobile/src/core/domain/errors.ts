/**
 * Typed domain errors + user-friendly descriptions, matching the web build.
 */
  ...
  if (err instanceof DomainError) {
    switch (err.code) {
      ...
      case 'AUTH_ACCOUNT_LOCKED':
        return 'Account locked for security. Try again in a few minutes.'
      ...
      case 'READING_OUT_OF_RANGE':
        return 'Meter reading is out of range.'
      case 'CLOSING_BELOW_OPENING':
        return `Closing reading for ${err.field ?? 'this fuel'} must not be lower than opening reading.`
