...
import { Building2, Check, MapPin, X, Fuel, ArrowRight, Flame } from 'lucide-react'
...
export const CompanySelectorModal: React.FC<CompanySelectorModalProps> = ({ isOpen, onClose }) => {
  ...
  const comp = COMPANIES_DIRECTORY.find(c => c.id === selectedCompId) || activeCompany

  const handleApply = () => {
    switchCompany(selectedCompId)
    switchStation(selectedStnId)
    onClose()
  }
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-slate-950 border-b border-slate-800">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400">PetroView Multi-Tenant Enterprise Switcher</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto flex flex-col gap-5">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">
              1. Choose Oil Marketing Company (OMC)
            </label>
            <div className="grid grid-cols-2 gap-3">
              {COMPANIES_DIRECTORY.map((c) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setSelectedStnId(c.stations[0]?.id || '')
                    }}
                  ...
                  >
                    ...
                    <span className="text-[10px] text-slate-400">{c.stations.length} Branches</span>
                  </button>
                )
              })}
            </div>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-slate-400">{stn.location} • Code: {stn.code}</p>
                      </div>
                    </div>
